...
const OUTPUT_PATH = join(__dirname, "..", "src", "generated", "news.json");
...
const normalizeEntry = (entry) => {
  ...
  return {
    ...
    body: extractBody(fields.body),
    rawBody: fields.body ?? null,
    publishedAt: fields.publishedAt ?? fields.publishedDate ?? null,
    heroImage: image
      ? {
          url: image.url.startsWith("//") ? `https:${image.url}` : image.url,
          width: image.details?.image?.width ?? null,
          height: image.details?.image?.height ?? null,
...
try {
  ...
  console.log(`Wrote ${items.length} articles to ${OUTPUT_PATH}`);
} catch (error) {
  console.error("Failed to pull news from Contentful:", error?.message ?? error);
  process.exit(1);
}
